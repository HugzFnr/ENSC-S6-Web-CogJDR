<?php
    ...
    if (!isset($_REQUEST['message_text'])) { // si pas de messages, c'est qu'on est en train de recharger les messages
        ...
        $r = sql_select(
            ...
            array('id_equipe' => $liste_id_equipe),
        ...
        while($message = $r->fetch()) {
            ...
            <li class="discussion_message"><b class="discussion_debut">[<?=$message['horaire_publi']?>] <a href="./compte.php?action=afficher&id=<?=$utilisateur['id_utilisateur']?>"><?=$utilisateur['pseudo']?></a> : </b><i class="discussion_texte"><?=$message['texte']?></i></li>
        <?php }
